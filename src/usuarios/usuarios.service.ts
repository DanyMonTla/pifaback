...
export class UsuariosService {
  ...
  const nuevo = this.usuarioRepo.create({
    ...
    habilitado: dto.bhabilitado,
    fechaAlta: new Date(dto.dfecha_alta),
    fechaBaja: dto.dfecha_baja ? new Date(dto.dfecha_baja) : null,
  });
...
async findAll(): Promise<any[]> {
  ...
  try {
    ...
    const fechaAlta = dto.dfecha_alta && dto.dfecha_alta !== "" ? new Date(dto.dfecha_alta) : usuario.fechaAlta;
    const fechaBaja = dto.dfecha_baja && dto.dfecha_baja !== "" ? new Date(dto.dfecha_baja) : usuario.fechaBaja;
    ...
    usuario.habilitado = dto.bhabilitado !== undefined ? Boolean(dto.bhabilitado) : usuario.habilitado;
